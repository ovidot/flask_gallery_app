...
def qry():
    cart_items = db.session.query(Cart).filter(
        User.user_id == Cart.cart_userid).all()
...
def home():
    ...
    if id:
        ...
        cart_items = db.session.query(Cart).filter(
            User.user_id == Cart.cart_userid).all()
        cat = db.session.query(Category).order_by(
            Category.category_name.asc()).all()
        all_arts = db.session.query(Artworks).order_by(
            func.rand()).limit(4).all()
        ...
        if uname != None and fname != None and lname != None and email != None and pwd != None and pwd != None and pwd == chkpwd:
            ...
            if duplicate_username > 0:
                ...
                if duplicate_email > 0:
                    ...
                    u = User(user_username=uname, user_fname=fname, user_lname=lname,
                             user_email=email, user_pwd=hashed_pwd, user_phone=phone)
...
def check_username():
    ...
    if usname != None:
        exists = db.session.query(User).filter(
            User.user_username == usname).scalar() is not None
        return str(exists)
    else:
...
def check_email():
    ...
    if email != None:
        exists = db.session.query(User).filter(
            User.user_email == email).scalar() is not None
        return str(exists)
    else:
        return "Enter Username"
...
def user_login():
    if session.get('user') != None:
        ...
        if request.method == 'GET':
            ...
            uname = request.form.get('username')
            pwd = request.form.get('password')
            uobj = db.session.query(User).filter(
                User.user_username == uname).first()
            if uobj != None:
...
def profile():
    if session.get('user') == None:
        ...
        fav = db.session.query(Favorites).filter(
            Favorites.fav_userid == id).limit(3).all()
        cart_items = db.session.query(Cart).filter(
            User.user_id == Cart.cart_userid).all()
        myworks = db.session.query(Artworks).filter(
            Artworks.upload_userid == id).all()
    ...
    if request.method == 'GET':
        ...
        if dob != "" and bio != "" and filename != "":
            ...
            if ext.lower() in allowed:
                ...
                # it will upload the picture and save it as picture.png,save it as the original filename
                file.save("galleryapp/static/uploads/"+newname)
                ...
                flash(
                    "<div class='alert alert danger'>'please use accepted file type'</div>")
        else:
            flash(
                "<div class='alert alert-danger'>Profile not updated succesfully check all fields</div>")
    ...
    if session.get('user') == None:
        ...
        if title != '' and artist != '' and categoryid != '' and price != '' and medium != '' and year != '' and dimension != '' and filename != "" and material != '' and subject != '' and style != '':
            ...
            if ext.lower() in allowed:
                ...
                a = Artworks(art_title=title, art_year=year, artist_fullname=artist, art_pix=newname, art_price=price, art_dimension=dimension,
                             upload_userid=userid, art_categoryid=categoryid, art_mediumid=medium, art_subjectid=subject, art_materialid=material, art_styleid=style)
                # add to session
                db.session.add(a)
                db.session.commit()
                artid = Artworks.query.order_by(Artworks.art_id.desc()).first()
                art_id = artid.art_id
                ap = Product_category(product_id=art_id, category_id=categoryid, medium_id=medium,
                                      subject_id=subject, style_id=style, material_id=material)
                db.session.add(ap)
                db.session.commit()
...
def update_product():
    ...
    if request.method == 'GET':
        id = session.get('user')
        deets = db.session.query(User).filter(User.user_id == id).first()
        all_arts = db.session.query(Artworks).filter(
            Artworks.upload_userid == id).all()
        cat = db.session.query(Category).order_by(
            Category.category_name.asc()).all()
...
def orders():
    if session.get('user') == None:
        ...
        if request.method == 'GET':
            id = session.get('user')
            all_orders = db.session.query(Orders).filter(
                Orders.order_userid == id).all()
            cat = db.session.query(Category).order_by(
                Category.category_name.asc()).all()
            deets = db.session.query(User).filter(User.user_id == id).first()
...
def order_deets(oid):
    if session.get('user') == None:
        ...
        id = session.get('user')
        all_orders = db.session.query(Orders).filter(
            Orders.order_id == oid).all()
        cat = db.session.query(Category).order_by(
            Category.category_name.asc()).all()
        order_deet = db.session.query(Order_details).filter(
            Order_details.orderdetails_orderid == oid)
...
def prod_details(idd):
    if request.method == 'GET':
        ...
        artobj = db.session.query(Artworks).filter(
            Artworks.art_id == idd).first()
...
def browse_all(page_num):
    id = session.get('user')
    deets = db.session.query(User).get(id)
    all_fav = db.session.query(Favorites).filter(
        Favorites.fav_userid == id).all()
    fav = []
    for j in all_fav:
    ...
    all_arts = db.session.query(Artworks).filter(
        Artworks.art_status == '1').all()
    all_art = db.session.query(Artworks).filter(
        Artworks.art_status == '1').paginate(per_page=9, page=page_num, error_out=True)
    ...
    mate = db.session.query(Material).order_by(
        Material.material_name.asc()).all()
...
def fil(idd):
    if request.method == 'GET':
        ...
        cart_items = db.session.query(Cart).filter(
            User.user_id == Cart.cart_userid).all()
        filtered = Product_category.query.filter(
            Product_category.category_id == idd).all()
        items = {}
        # all_arts= db.session.query(Artworks).order_by(func.rand()).all()
        all_arts = db.session.query(Artworks).filter(
            Artworks.art_status == '1').all()
        ...
        cat = db.session.query(Category).order_by(
            Category.category_name.asc()).all()
        med = db.session.query(Medium).order_by(Medium.medium_name.asc()).all()
        subj = db.session.query(Subject).order_by(
...
def prod_more(idd):
    if request.method == 'GET':
        ...
        cart_items = db.session.query(Cart).filter(
            User.user_id == Cart.cart_userid).all()
        category = db.session.query(Category).all()
        works = db.session.query(Artworks).filter(
            Artworks.upload_userid == idd).all()
        cat = db.session.query(Category).order_by(
            Category.category_name.asc()).all()
...
def artist(idd):
    ...
    for j in all_fav:
        fav.append(j.fav_artid)
    all_arts = db.session.query(Artworks).filter(
        Artworks.upload_userid == idd).order_by(func.rand()).all()
    deets = db.session.query(User).get(id)
    for p in all_arts:
        pass
...
def add_favorites(id):
    if session.get('user') != None:
        ...
        artid = id
        chk = db.session.query(Favorites).filter(
            Favorites.fav_userid == userid).filter(Favorites.fav_artid == artid).first()
        if chk == None:
...
def cart():
    if session.get('user') == None:
        ...
        cat = db.session.query(Category).order_by(
            Category.category_name.asc()).all()
        cart_total = db.session.query(func.sum(Cart.cart_itemprice)).filter(
            Cart.cart_userid == id).first()
        cart_items = db.session.query(Cart).filter(
            Cart.cart_userid == id).all()
        all_arts = db.session.query(Artworks).order_by(func.rand()).all()
...
def add_cart(add):
    if session.get('user') == None:
        ...
        id = session.get('user')
        deets = db.session.query(User).get(id)
        price_obj = db.session.query(Artworks).filter(
            Artworks.art_id == add).first()
        price = price_obj.art_price
        cartitemid = db.session.query(Cart).filter(
            Cart.cart_artid == add).all()
        for c in cartitemid:
...
def make_payment():
    if session.get('user') == None:
        ...
        if request.method == 'GET':
            ...
            cart_total = db.session.query(func.sum(Cart.cart_itemprice)).filter(
                Cart.cart_userid == id).first()
            o = Orders(order_amt=cart_total[0], order_userid=id,
                       order_address=user.user_address, order_phone=user.user_phone)
            db.session.add(o)
            db.session.commit()
...
def confirm():
    if request.method == 'GET':
        ...
        order_deets = db.session.query(Order_details).filter(
            Order_details.orderdetails_orderid == session['order_id']).all()
        ...
        p = Payment(payment_orderid=session['order_id'],
                    payment_refno=session['reference'], payment_userid=session['user'])
        db.session.add(p)
        db.session.commit()
...
def address():
    if request.method == 'GET':
        ...
        o = Orders(order_amt=cart_total[0], order_userid=id,
                   order_address=address, order_phone=phone)
        db.session.add(o)
        db.session.commit()
        last_order = Orders.query.order_by(Orders.order_id).filter(
            Orders.order_userid == id).first()
...
def paystack():
    ...
    if refid == None:
        ...
        if rspjson['status'] == True:
            # update Order table
            o = db.session.query(Orders).filter(
                Orders.order_userid == id).order_by(Orders.order_id.desc()).first()
            o.order_status = 'success'
            db.session.commit()
            # update payment table
            p = db.session.query(Payment).filter(Payment.payment_userid == id).order_by(
                Payment.payment_orderid.desc()).first()
            p.payment_amt = rspjson['data']['amount']
            p.payment_status = rspjson['data']['status']
            db.session.commit()
            # update Artwork availability status table
            od = db.session.query(Order_details).filter(
                Order_details.orderdetails_orderid == o.order_id).all()
            for j in od:
                art_av = db.session.query(Artworks).filter(
                    j.orderdetails_artid == Artworks.art_id).one()
